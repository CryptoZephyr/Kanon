...
- `DEPLOYMENT.md` records the live Render and Neon services, the implemented frontend state, the absent Kanon Vercel project, and the T17 Vercel deployment boundary.
...
- The initial structure intentionally deferred API and frontend implementation. That phase-boundary constraint is complete now: `apps/api` and `apps/web` are implemented, deployed backend services are live, and the public Vercel frontend deployment remains the T17 action.
...
- The backend deployment happened before the frontend implementation pass. Both T15 and T16 are complete, so the ordering difference is historical and no longer a blocker. T17 now owns the Vercel frontend deployment, demo rehearsal, and evidence freeze.
...
The repository is on branch `main` with the private remote `https://github.com/CryptoZephyr/Kanon`. The dependency lockfile is present. The current working tree contains the verified T1A domain foundation, final T1B authority model, T2 permission diff engine, T5 execution-method-aware Privy compiler, T6 ENSv2 write probe, T7 ENS identity adapter, T8 and T9 lifecycle contracts, T10 isolated runner, T11 to T13 live lifecycle probe, the T14 API contract module and tests, the T15 approved frontend, Render and Neon deployment configuration, deployment evidence, frontend lifecycle evidence, the Vercel deployment record, read-only documentation evidence, live Privy and ENS write/read/permission evidence, isolated Privy spike harnesses, lifecycle evidence, and synchronized documentation edits. `.env.local` remains ignored and no secret value is present in tracked files. All ENS writes remain within the owner-authorized Sepolia hackathon namespace. T11 and T12 updated only the protected records through the authorized company writer. T13 changed only `kanon.status` to `revoked`; the persistent identity records remain readable. No mainnet or production ENS write was performed. The public Vercel deployment is still pending as the T17 immediate action.

## Exact next action

T15 and T16 are complete. The exact next action is T17 Vercel frontend deployment, demo rehearsal, and evidence freeze. Create or link the Kanon Vercel project, deploy `apps/web`, and provide a server-side `/api` proxy that attaches `KANON_COMPANY_API_TOKEN` from Vercel environment state. Do not expose that token through a `VITE_` variable or a static rewrite. After deployment, verify the production frontend against the live Render API and run the complete disposable create to revoke flow. Keep all ENS work on the authorized Sepolia namespace only.
